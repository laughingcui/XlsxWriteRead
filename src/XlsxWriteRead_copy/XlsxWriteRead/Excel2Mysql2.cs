...
namespace XlsxWriteRead
{
    ...
    public class Excel2Mysql2
    {
        ...
        /// 存储所有打开的xls
        /// </summary>
        static List<SxlsFile> xlsFilesList = new List<SxlsFile>();
        ...
        /// <returns></returns>
        public static bool xls2mysql(List<string> files)
        {
            ...
            try
            {
                for (int i_file = 0; i_file < fileCount; i_file++)
                {
                    ...
                    string fileChName = path_all.Substring(path_all.LastIndexOf("\\") + 1, path_all.LastIndexOf(".") - (path_all.LastIndexOf("\\") + 1));
                    string filePYName = "";
                    //string sheetPYName = "";
            ...
            int xlsCount = xlsFilesList.Count;
            try
            {
                for (int i_L1table = 0; i_L1table < xlsCount; i_L1table++)
                {
                    //1-1. create L1 db table
                    SxlsFile xlsFile = xlsFilesList[i_L1table];
                    string sql_create_table = "create table " + xlsFile.fileName + " (id int(11) NOT NULL auto_increment, sheet varchar(60) not null, primary key(id))ENGINE= MYISAM CHARACTER SET utf8;";  //id自动增长, 一级表表名 = 文件名
                    ...
                    for (int i_L1table_value = 0; i_L1table_value < sheetCount; i_L1table_value++)
                    {
                        ...
                        string sql_insert = "insert into " + xlsFile.fileName + "(sheet) values(";
                        sql_insert += "'" + xlsFile.sheetsList[i_L1table_value].sheetName + "');";
                        MySqlHelper.ExecuteNonQuery(MySqlHelper.Conn, System.Data.CommandType.Text, sql_insert);

                        // 2-1. create L2 db table
                        Ssheet sheetData = xlsFile.sheetsList[i_L1table_value];
